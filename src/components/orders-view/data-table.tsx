...
const PaginationSection = <TData extends OrderRow>({ table }: { table: TableInstance<TData> }) => {
	return (
		<div className='flex flex-col gap-4 px-2 py-2 sm:flex-row sm:items-center sm:justify-between'>
			<div className='order-2 whitespace-nowrap text-sm text-muted-foreground sm:order-1'>
				{table.getFilteredRowModel().rows.length === table.getCoreRowModel().rows.length ? (
					...
					<>
						Filtered Records: {table.getFilteredRowModel().rows.length} / {table.getCoreRowModel().rows.length}{" "}
						total
					</>
				)}
			</div>
...
const DataTable = React.memo(<TData extends OrderRow, TValue>({ columns, data }: DataTableProps<TData, TValue>) => {
